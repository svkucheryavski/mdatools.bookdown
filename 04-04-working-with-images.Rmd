...
By default image for one channels is shown using jet color palette for intensities, but you can also use
gray colors, palette from colorbrewer2 as well as your own.
...
`imshow()` function (or actually they are shown as dark gray pixels), however providing argument 
`show.excluded = TRUE` you can make them visible. Note that once you exclude any pixels you can not use 
method `mda.setimbg()`, so define the background pixels first.
