...
```{block, type='caution'}
Historically, the distances are called in this package and also in some other software as *Residual distances* or just *residuals*, which is not fully correct. Therefore functions, which visualize the corresponding distances, are called `plotResiduals()` (`plotXResiduals()`, `plotYResiduals()`, `plotXYResiduals()` in PLS based methods). However in the text of the tutorial we will refer to them as distances most of the time.
```
