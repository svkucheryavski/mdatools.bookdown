...
```{block, type='caution'}
You can think that SIMCA is actually a PCA model where function [`categorize()`][Categorization of data rows based on critical limits] is used to make a decision: if object is categorized as regular, it will be considered as member of the class, otherwise — it is a stranger. Therefore read carefully how PCA [works in general][Principal component analysis] and how critical limits for distances [are computed][Distances and critical limits] in particular, to understand how SIMCA works.
```
