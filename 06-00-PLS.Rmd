...
regression coefficients. The values for regression coefficients are available in `m.regcoeffs.values`, it is an array
...
confidence intervals and other statistics for the coefficients using Jack-Knife method (will be shown later), which produces
extra entities.
...
You can get the corrected coefficients, which can be applied directly to the raw data, by using
method `getRegcoeffs()`:
...
```
And here is the RMSE plots (they are identical of course):
...
If you want more details run `summury()` for one of the result objects.
...
by using parameter `show.ci = FALS`.
...
Function `getRegcoeffs()` in this case may also return a column with p-values, if user specifies a parameter `full`.

```{r}
show(getRegcoeffs(mjk, ncomp = 2, full = T))
```

The p-values are stored as a 3-way array similar to regression coefficients. The selection can be made by comparing e.g. 
