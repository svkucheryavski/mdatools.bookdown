...
Well, in this case both pointed on the same number, 5, but sometimes they give different solutions.
...
In the previous examples we set arameter `cv` to a number 1. But in fact it can be a number, a list or a vector with segment values manually assigned for each measurement.
...
As you can see, there are no values for cumulative explained variance for cross-validation result, because in case of cross-validation it is not possible to compute the variance correctly. But the other figures of merit are shown.
