...
In case of PLS1 (when there is only one y-variable), $N_z = 1$. Otherwise, both $z_0$ and $N_z$ are computed using [data driven approach][Data driven approach], similar to $q_0$, $N_q$, $h_0$, $N_h$, using either classical (based on moments) or robst approach.
...
Here $N_g = N_f + N_z$. This approach was proposed by Rodionova and Pomerantsev and described in [this paper](https://pubs.acs.org/doi/10.1021/acs.analchem.9b04611). The distances and the critical limits can be visualized in `plotXYResiduals()`:

```{r, fig.width = 9, fig.height = 5}
par(mfrow = c(1, 2))
plotXYResiduals(m)
plotXYResiduals(m, ncomp = 2, log = TRUE, show.labels = TRUE, labels = "indices")
```

It can be used to detect outliers in PLS by comparing the total distance, $g$, with critical limit computed for predefined significance level, $\gamma$:

$$g > \chi^{-2} \big( (1 - \gamma)^{1/I}, N_g \big) $$

The proper procedure for detection and removing outliers can be found in the [this](https://pubs.acs.org/doi/10.1021/acs.analchem.9b04611). The detection can be done using function `categorize()` which works similar to the same function for PCA decomposition — returns vector with categories for each measurement, as shown in an example below:

```{r}
c = categorize(m, m$res$cal, ncomp = 2)
print(c)
```
