...
The purity based approach implemented in *mdatools* was [proposed](https://pubs.acs.org/doi/abs/10.1021/ac00014a016#) by Willem Windig and Jean Guilment in 1991 as a simple alternative to interactive self-modeling multivariate analysis technique. The general idea of the approach is to find variables (wavelength, wavenumbers, etc.) in $\mathbf{D}$, which are influenced mostly by one chemical component. Such variables are called as *pure variables*. If we can identify pure variable for each of the pure components, then we can solve the MCR problem by using ordinary least squares method:
...
Here $\mathbf{\hat{S}}$ and $\mathbf{\hat{C}}$ are the spectra and concentrations of the pure components estimated by this method. The matrix $\mathbf{D}_R$ is a reduced version of $\mathbf{D}$ where only pure variables are kept (one for each pure component, so this matrix has a dimension $A \times A$). So the question is how to find the pure variables?

Windig and co-authors proposed to do it by computing a *purity* of a variable $j$ as follows:

$$p_j = \frac{\sigma_j}{\mu_j + \alpha}$$

Here $\sigma_j$ and $\mu_j$ are standard deviation and mean values computed for the variable ($j^{th}$ column of the matrix $\mathbf{D}$). Computing the purity for all variables in $\mathbf{D}$ results in a purity spectrum, $\mathbf{p}$.

Parameter $\alpha$ is computed as 1–10% of a maximum of all mean values. More details about the method can be found both in the [original paper](https://pubs.acs.org/doi/abs/10.1021/ac00014a016#) as well as in one of the [recent reviews](https://www.sciencedirect.com/science/article/pii/B9780444636386000036).


### An example {-}

Function `mcrpure()` implements the purity based method in *mdatools*. It has two mandatory arguments — matrix with the original spectra and number of pure components — as well as several extra parameters. The most important parameter is the `offset` which defines value $\alpha$ in the purity spectrum. Thus `offset = 0.01` will compute $\alpha$ as 1% of the maximum mean value. The default offset is 0.05.
...
summary(m)
```
The summary shows explained variance, cumulative explained variance, index of pure variable and its purity value for each of the three components. For example, in case of the first component, the purest variable is located in column 782 and its purity is equal to 0.639. As one can see, current solution explains 99.6% of the total variance.

Any MCR method in *mdatools* contains the resolved spectra (as `m$respec`) and contributions (as `m$rescont`) as well as some extra data, e.g. purity spectra for each component, etc. You can see a list of all objects available by simply printing the object with *mcrpure* results:
...
The vertical dashed lines on the purity spectra plot show the selected pure variables. As we can see, indeed they correspond to the largest values in the corresponding purity spectrum (so have the highest purity). We can also notice that when offset is small (top plots, 1%) the purity spectra look quite noisy. On the other hand, using too large offset can lead to a selection of less pure variables, so this parameter should be selected with caution. In most cases, having offset around 5% of the largest mean value allows to achieve satisfactory results. But it is always a good idea to vary the parameter and investigate all plots before the final decision.
...
S1.hat = prep.norm(mda.t(m1$resspec), "length")
S2.hat = prep.norm(mda.t(m2$resspec), "length")
...
for (a in 1:3) {
   s = mda.subset(S, a)
   s1.hat = mda.subset(S1.hat, a)
   s2.hat = mda.subset(S2.hat, a)
   mdaplotg(list(orig = s, resolved = s1.hat), type = "l", col = col, lwd = lwd)
   mdaplotg(list(orig = s, resolved = s2.hat), type = "l", col = col, lwd = lwd)
}
```
...
One of the ways to improve the result of resolution based on the purity approach is to use derivatives of the spectra. This can be especially efficient if spectra show broad overlapping peaks, e.g. in case of NIR or UV/Vis spectra.

The derivatives in `mcrpure()` can be used either only for detection of pure variables or for both  detection of pure variables and the resolving. To use derivatives one has to specify two parameters. First, `use.deriv`, is a number between 0 and 2 (0 — do not use, 1 — use only for detection of pure variables, 2 — use for both the detection and the resolution). Second parameter, `savgol` is a list with Savitzky-Golay settings (see `?prep.savgol` for details).

The code below shows the result of applying `mcrpure()` to the *Simdata*, which consists of 150 UV/Vis spectra with very broad peaks.

```{r, fig.width = 9, fig.height = 9}
   data(simdata)

   # define Savitzky-Golay parameters
   savgol = list(porder = 2, dorder = 2, width = 5)

   # get spectra and resolve them with and without derivatives
   D <- simdata$spectra.c
   m1 <- mcrpure(D, ncomp = 3)
   m2 <- mcrpure(D, ncomp = 3, offset = 0.04, use.deriv = 2, savgol = savgol)
...
Using the default settings (left plots) does not allow to get any meaningful solutions (you can notice, for example, negative peaks). However, combination of derivatives and adjustment of the offset, results in a very good outcome — the resolved spectra are very similar to the original ones (original spectra are not shown on the plots).
