...
To compute the probability we use the theoretical distribution for Q and T^2^ distances as for computing critical values (defined by the parameter `lim.type`). The distribution is used to calculate a p-value — chance to get object with given distance value or larger. The p-value is then compared with signidicance level, $\alpha$, and the probability, $\pi$ is calculated as follows:
...
In case of rectangular acceptance area (`lim.type = 'jm'` or `'chisq'`) the probability is computed separately for Q and T^2^ values and the smallest of the two is taken. In case of triangular acceptance area (`lim.type = 'ddmoments'` or `'ddrobust'`) the probability is calculated for a combination of the distances.
